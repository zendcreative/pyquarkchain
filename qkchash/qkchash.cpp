...
void qkc_hash(
    ...
    for (uint32_t i = 0; i < ACCESS_ROUND; i ++) {
        ...
        for (uint32_t j = 0; j < mix.size(); j++) {
            uint64_t p = fnv64(i ^ seed[0], mix[i % mix.size() + j]);
    ...
    // Truncate mix directly.  Similar to ETH, compression and post-sha3 can be applied.
...
int main() {
    ...
    org::quarkchain::generate_init_set(oset, 1, org::quarkchain::INIT_SET_ENTRIES);
    auto used_time = std::chrono::high_resolution_clock::now() - t_start;
    std::cout << "Generate time " << std::chrono::duration<double, std::milli>(used_time).count() << std::endl;

    t_start = std::chrono::high_resolution_clock::now();
    ordered_set_t noset = oset;
    used_time = std::chrono::high_resolution_clock::now() - t_start;
    std::cout << "Copy time " << std::chrono::duration<double, std::milli>(used_time).count() << std::endl;
